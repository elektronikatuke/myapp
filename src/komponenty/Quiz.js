...
const Quiz = ({questions}) => {
    ...
    function correct(answerOption){
        ...
        if(answerOption.isCorrect){
            document.getElementById(answerOption.kluc).setAttribute("class", "true");
        }
	...
	const AnswerClick = (isCorrect, id, answerText, kluc) => {
		...
		if(number.find(element => element === id)){
			...
			if(text.find(odpoved => odpoved === answerText)){
				...
				const doc = document.getElementById(kluc);
					...
					<div>
						<div className="flex_left">
							...
							<div className='answer-section'>
								{questions.answerOptions.map((answerOption) => (
                                    <>
									<button id = {answerOption.kluc} onClick={() => {
										AnswerClick(answerOption.isCorrect, questions.id, answerOption.answerText, answerOption.kluc);
							...
							<div className='next' id={"9" + questions.id}>	
								<svg width="58" height="89" viewBox="0 0 58 89" fill="none">
